defmodule HgsIdeationWeb.WorkflowLiveTest do
  ...
  test "renders workflow statuses, transitions, and Mermaid output", %{conn: conn} do
    ...
    assert has_element?(view, "#workflow-diagram")
    assert has_element?(view, "#workflow-diagram-status-review")
    assert has_element?(view, "#workflow-diagram-transition-review-done")
